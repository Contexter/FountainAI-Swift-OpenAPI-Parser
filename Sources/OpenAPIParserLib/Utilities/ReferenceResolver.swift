...
class ReferenceResolver {
    ...
    func resolve<T: Codable>(ref: String) -> T? {
        ...
        case "schemas":
            return (components.schemas?[componentName] as? SchemaObject as? T as? T) as? T
        case "responses":
            return (components.responses?[componentName] as? ResponseObject as? T as? T) as? T
        case "parameters":
            return (components.parameters?[componentName] as? ParameterObject as? T as? T) as? T
        case "examples":
            return components.examples?[componentName] as? T as? T as? T
        case "requestBodies":
            return components.requestBodies?[componentName] as? T as? T as? T
        case "headers":
            return components.headers?[componentName] as? T as? T as? T
        case "securitySchemes":
            return components.securitySchemes?[componentName] as? T as? T as? T
        case "links":
            return components.links?[componentName] as? T as? T as? T
        case "callbacks":
            return components.callbacks?[componentName] as? T as? T as? T
        default:
